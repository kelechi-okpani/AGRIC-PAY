...
import { startCardWorker }         from './jobs/card.job';
import { startCryptoWorker }       from './jobs/crypto.job';
...
import './events/handlers/transfer.handler';
import './events/handlers/order.handler';
...
app.use('/api',           depositWithdrawalRoutes);  // webhooks + deposit/withdraw REST

// ── Health check ──────────────────────────────────────────────────────────
app.get('/health', (_req, res) =>
  res.json({ status: 'ok', app: env.APP_NAME, version: '1.0.0' })
);
...
const bootstrap = async () => {
  ...
  httpServer.listen(env.PORT, () => {
    ...
    logger.info(`💬 WhatsApp webhook: http://localhost:${env.PORT}/api/whatsapp/webhook`);
